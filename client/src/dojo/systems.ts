...
export function systems({
  ...
  const getToastAction = (transaction_hash: string) => {
    return {
      ...
      onClick: () =>
        window.open(
          `https://worlds.dev/networks/slot/worlds/retropie/txs/${transaction_hash}`,
        ),
    };
  };

  const toastPlacement = "bottom-center";
  ...
  const register = async ({ account, ...props }: SystemTypes.Register) => {
    await handleTransaction(
      account,
      () => client.actions.register({ account, ...props }),
      "Game has been registered.",
    );
  };

  const update = async ({ account, ...props }: SystemTypes.Update) => {
    await handleTransaction(
      account,
